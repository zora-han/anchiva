...
$(document).ready(function () {
    ...
    dataPicker();
    setInterval(function(){
        var datatime=$('.form_datetime');
        datatime.datetimepicker('setStartDate', new Date(new Date()-90*24*3600*1000).toLocaleString().replace(/\//g,'-').replace(/[上下午]{2}/,""));
        datatime.datetimepicker('setEndDate', new Date().toLocaleString().replace(/\//g,'-').replace(/[上下午]{2}/,""));
    },10000);
    //表格按钮切换
    ...
    $(".sub-menu>.nav-item a").click(function(e){
        ...
        if(href==="#system-state"){
            ...
            click=setTimeout(timePicker(href.slice(1)),200);
        }else if(href==="#monitoring-defense"){
            //第一行的三个数据
            dataM();
            clearInterval(time1);
            time1=setInterval(dataM,1000);
            //威胁安全趋势
            securityTrend();
            attackSourceRanking();
            ...
            timePicker(href.slice(1));
        }else if(href==="#WAF"){
            //第一行的三个数据
            dataM1();
            clearInterval(time2);
            time2=setInterval(dataM1,1000);
            //威胁安全趋势
            securityTrend1();
