...
var filter,filter1,filter2,filter3={},serverGroupAdd=[],HTTPParametersProtectiveAdd={},
    ...
    operationIntrusion='<td><img src="imgs/9.png" alt=""/></td><td><img src="imgs/4.png" alt=""/></td>',
    //web扫漏页
    ...
    operationStrategy ='<td><img src="imgs/7.png" alt="start"/></td><td><img src="imgs/9.png" alt=""/></td><td><img src="imgs/4.png" alt=""/></td>',
    //早前页面的操作标识
    ...
    policyReferences={
        ...
        'cur_page':1,		/*当前页号*/
        'data':[
            {id:1,source:"XXX",topInterface:"XXX",bottomInterface:"XXX",webProtective:"XXX",invasion:"XXX",attack:"XXX"}
        ]
    },
    //流量统计
    trafficStatistics={
        'record_count':10,	/*总记录数*/
        'page_size':10,		/*每页的记录数*/
        'page_count':1,	    /*总页数*/
        'cur_page':1,		/*当前页号*/
        'data':[
            {id:1,ip:'123.125.80.78',access:6,accessRate:'100%',attack:0,attackRate:'0.00%',crawler:0,crawlerRate:'0.00%',total:6},
            {id:2,ip:'173.194.72.101',access:5,accessRate:'100%',attack:0,attackRate:'0.00%',crawler:0,crawlerRate:'0.00%',total:5},
            {id:3,ip:'123.125.82.253',access:4,accessRate:'100%',attack:0,attackRate:'0.00%',crawler:0,crawlerRate:'0.00%',total:4},
            {id:4,ip:'56.68.227.107',access:4,accessRate:'100%',attack:0,attackRate:'0.00%',crawler:0,crawlerRate:'0.00%',total:4},
            {id:5,ip:'173.194.72.100',access:4,accessRate:'100%',attack:0,attackRate:'0.00%',crawler:0,crawlerRate:'0.00%',total:4},
            {id:6,ip:'62.2.251.193',access:3,accessRate:'100%',attack:0,attackRate:'0.00%',crawler:0,crawlerRate:'0.00%',total:3},
            {id:7,ip:'123.125.80.93',access:3,accessRate:'100%',attack:0,attackRate:'0.00%',crawler:0,crawlerRate:'0.00%',total:3},
            {id:8,ip:'162.118.31.226',access:2,accessRate:'100%',attack:0,attackRate:'0.00%',crawler:0,crawlerRate:'0.00%',total:2},
            {id:9,ip:'162.118.9.203',access:2,accessRate:'100%',attack:0,attackRate:'0.00%',crawler:0,crawlerRate:'0.00%',total:2},
            {id:10,ip:'69.192.4.66',access:2,accessRate:'100%',attack:0,attackRate:'0.00%',crawler:0,crawlerRate:'0.00%',total:2}
        ]
    },
    //web缓存
    webCaching={
        ...
        'cur_page':1,		/*当前页号*/
        'data':[
            {id:1,start:'2010-06-29 09:05:20',url:"172.17.1.130/ftp/automaller.log",port:80,cacheTime:'2009-01-04 22:04:41',serverTime:"2009-01-04 22:04:14"}
        ]
    },
    //WEB威胁模拟数据
    threatSimulation=[
        {id:1,url:"172.196.128.131",attack:1,rate:"10%"},
        {id:2,url:"172.196.128.132",attack:1,rate:"10%"},
        {id:3,url:"172.196.128.133",attack:1,rate:"10%"},
        {id:4,url:"172.196.128.134",attack:1,rate:"10%"},
        {id:5,url:"172.196.128.135",attack:1,rate:"10%"},
        {id:6,url:"172.196.128.136",attack:1,rate:"10%"},
        {id:7,url:"172.196.128.137",attack:1,rate:"10%"},
        {id:8,url:"172.196.128.138",attack:1,rate:"10%"},
        {id:9,url:"172.196.128.139",attack:1,rate:"10%"},
        {id:10,url:"172.196.128.130",attack:1,rate:"10%"}
    ];
function tab(idName){
    //病毒感染源
    var virus = {
    ...
    switch (idName){
        ...
        case "strictCharacteristics":return basicCharacteristics;
        case "customCharacteristics":return basicCharacteristics;
        ...
        case "policyReferences":return policyReferences;
        case "trafficStatistics":return trafficStatistics;
        case "webCaching":return webCaching;
        case "trojanMonitoring":return threatSimulation;
        case "WebShellMonitoring":return threatSimulation;
        case "informationLeakageMonitoring":return threatSimulation;
    }
}
function addData(name){
    var tabName=tab(name),//获取所需数据
        ...
        index,
        i;//循环起始点
    if(name==="trojanMonitoring"||name==="WebShellMonitoring"||name==="informationLeakageMonitoring"){
        ...
        $.each(tabName.data,function(i,n){
            ...
            + n.port +'</td><td>'
            + n.cacheTime +'</td><td>'
            + n.serverTime +'</td><td><img src="imgs/setDown.png"/></td><td><img src="imgs/setDown.png"/></td></tr>';
        });
    }else if(name==="trafficStatistics"){
        $.each(tabName.data,function(i,n){
            str+='<tr><td>'
            + n.id +'</td><td>'
            + n.ip +'</td><td>'
    ...
    }else if(name==="policyReferences"){
        $.each(tabName.data,function(i,n){
            str+='<tr><td><input type="checkbox" name="checkIP" ></td><td>'
            + n.source +'</td><td>'
            + n.topInterface +'</td><td>'
            + n.bottomInterface +'</td><td>'
            + n.webProtective +'</td><td>'
            + n.invasion +'</td><td>'
            + n.attack +'</td>'
            +operationStrategy+'</tr>';
        });
    }else if(name==="preventingHotlinking"){
        $.each(tabName.data,function(i,n){
            str+='<tr><td><input type="checkbox" name="checkIP" ></td><td>'
            + n.name +'</td><td>'
        ...
        $.each(tabName,function(i,n){
            str+='<tr><td><input type="checkbox" name="checkIP" ></td><td>'
            + n.name
            +'</td><td><select name="move" data-value="1"><option value="0">阻断</option><option value="1" selected>放行</option><option value="2">告警</option><option value="3">重定向</option></select></td><td><select name="log"><option value="0">记录</option><option value="1">不记录</option></select></td><td>'
            + n.count +'</td><td>';
            if(n.id<12){
    ...
    }else if(name==="basicCharacteristics"||name==="customCharacteristics"||name==="strictCharacteristics"){
        $.each(tabName,function(i,n){
            if(n.isParent===true){
            ...
            str+='<select name="move"><option value="0">阻断</option><option value="1" selected>放行</option><option value="2">告警或重定向</option></select></td><td><select name="log"><option value="0">记录</option><option value="1">不记录</option></select></td>'
...
function event(id){
    ...
    eventChart.on('legendselectchanged', function (params) {
        console.log($(this))
        console.log(params)
        // 获取点击图例的选中状态
        var isSelected = params.selected[params.name];
        if(params.name==="入侵防御"){
            params.selected["POP3"]=false
        }
        // 在控制台中打印
...
//5.12流量统计
function trafficStatisticsBar(){
    var trafficStatisticsChart = echarts.init(document.getElementById("trafficStatisticsBar")),
        trafficStatisticsSource=tab("trafficStatistics"),ip=[],data=[];
    $.each(trafficStatisticsSource.data,function(i,n){
        ip.push(n.ip);
        data.push(n.access);
    });
    var trafficStatisticsOption = {
        tooltip : {
            trigger: 'axis'
        },
        grid: {
        ...
        series : [
            {
                ...
                itemStyle: {
                    normal: {
                        color: function(params) {
                            var colorList = [
                                '#92C333','#E7B933','#29B3E6','#2AB136','#CC72FC',
                                '#F6A39C','#E6B832','#807BE5','#137BDD','#FC6E9C'
                            ];
                            return colorList[params.dataIndex]
                        }
                    }
    ...
    trafficStatisticsChart.setOption(trafficStatisticsOption);
    window.addEventListener("resize",trafficStatisticsChart.resize);
}
//5.13WEB威胁
function webThreats(id){
    var webChart = echarts.init(document.getElementById(id+"Pie")),
        pieData=tab(id),arr=[],legend=[];
    $.each(pieData,function(i,n){
        arr.push({
            value: n.attack,
            name: n.url
        });
    ...
    var webOption = {
        color:["#41C513","#F5DB00","#FABC00","#964FF7","#702AFF","#FCFF02","#4F9200","#6BB90F","#B0FA00","#56E723"],
        tooltip : {
            trigger: 'item',
            formatter: "{a} <br/>{b} : {c} ({d}%)"
        },
        ...
        series : [
            {
                name: function(){
                    if(id==="informationLeakageMonitoring"){
                        return "信息泄漏监测"
                    }else if(id==="WebShellMonitoring"){
                        return "WebShell监测"
                    }else{
                        return "挂马监测"
                    }
    ...
    window.addEventListener("resize",webChart.resize);//自适应分辨率改变重画图形
}
//5.14WEB缓存
function webCachingLine(id,time){
    var webCachingChart = echarts.init(document.getElementById(id)),
        interval,count,now,date = [],data = [];
    if(time===undefined||time===""){
        interval=60000;//每一分钟一个数据
        count=60;//一小时内有六十个数据
        ...
        series: [
            {
                ...
                areaStyle:{
                    normal:{
                        color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
                            offset: 0, color: '#CCF2A7' // 0% 处的颜色
                        }, {
                            offset: 1, color: '#F5F8FD' // 100% 处的颜色
                        }], false)
                    }
                },
                lineStyle:{
    ...
    webCachingChart.setOption(optionWebCaching);
    gengS();
    webCachingChart.timeTicket = setInterval(gengS,30000);
    window.addEventListener("resize",webCachingChart.resize);
}
...
function analysis(relationship,relationshipPie,me,source,filterName,analysisSelector){
    ...
    $("#"+source+" tbody,#"+filterName+" tbody").delegate("a", "click", function(e){//表格过滤事件
        e.preventDefault();
        var href=$(this)[0].hash.slice(1),//获取点击表格的href
            value=$(this).text(),//获取数据
            arr=[];
        ...
        $("#"+filterName).addClass("active").siblings("table").removeClass("active");//显示过滤表格
        addData(filterName);//给表格加数据
        $("#"+href+" p").text(value).parent().addClass("active");//显示过滤条件
    });
    $(".filter-conditions ul li i").click(function(){
        attackData=tab(source);
        var li=$(this).parent().siblings().has("i"),//过滤条件
            lArr=[],//过滤条件内容
            arr=[];
        $.each(li,function(i,n){
            (n.className==="active")&& lArr.push({name:n.id,value:n.childNodes[1].innerHTML});//name:所查数据名称，value：所查数据值
        });
        if(lArr.length==2){
            $.each(attackData.data,function(i,n){
                (n[lArr[0]["name"]]===lArr[0]["value"]&&n[lArr[1]["name"]]===lArr[1]["value"])&& arr.push(n);
            });
            attackData.data=arr;
...
//13.自定义特征库树，删除叶子
function DelTree(id,custom){
    $.each(custom, function (i, n) {
        if (n.name == selectInput[0].value && n.ty == selectInput[1].value) {
...
function heightSet(){
    ...
    if(wrapper.hasClass("content-wrapper")){
        ...
        tabContent.height(wrapper.height()-74);
    }
...
//21.1设置配置页初始高度
function iframeSet(){
    var iframe=$("body",parent.document).find('#WAF-strategy-add-configuration').children("iframe"),
        tabContent=$("body",parent.document).find('#application-security-policy').children(".page-content").children(".tab-content");
    iframe.height(tabContent.height()-5);
}
...
function checkSelect(){
    $("select").change(function(){
        ...
        if(fatherChecked){
            var chbs=$('#'+parent+' [name="'+childName+'"]'),
                value=$(this)[0].value;
            $.each(chbs,function(i,n){
                n.value=value;
            });
    ...
    $('[name="optionBtn"]').click(function(){
        ...
        if(name==="CSRF检查"){
            ...
            $(".tip-container2 .pop").addClass("active")
                ...
                .siblings("div").removeClass("active")
        }else if(name==="URL访问控制"){
            $("#WAF-strategy-add-configuration").addClass("active")
                .children("iframe").attr("src","data/security-policy/application-security-policy/WAF-strategy-add/content-security-pop/url-access-control.html")
...
function showDetail(event, treeId, treeNode, clickFlag) {
    ...
    if(treeNode.pId!==null){
        if(treeId==="treeType1"){//应用安全策略-基本特征库
            selectSpan[0].innerHTML = treeNode.id;
            selectSpan[1].innerHTML = treeNode.os;
            selectSpan[2].innerHTML = treeNode.name;
            selectSpan[3].innerHTML = treeNode.server;
            selectSpan[4].innerHTML = treeNode.l;
            selectSpan[5].innerHTML = treeNode.database;
            selectSpan[6].innerHTML = treeNode.ty;
            selectSpan[7].innerHTML = treeNode.language;
            selectSpan[8].innerHTML = treeNode.detailed;
        }else if(treeId==="treeType"){//入侵防护策略-基本特征库
            selectSpan[0].innerHTML = treeNode.name;
            selectSpan[1].innerHTML = treeNode.id;
            selectSpan[2].innerHTML = treeNode.l;
            selectSpan[3].innerHTML = treeNode.ty;
            selectSpan[4].innerHTML = treeNode.name;
        }else if(treeId==="treeType2"||treeId==="treeType3"){//自定义特征库
            selectInput[0].value = treeNode.name;
            selectSelect[0].value = treeNode.ty;
            selectSelect[1].value = treeNode.l;
            selectInput[1].value = treeNode.operation;
            selectTextarea[0].value = treeNode.detail;
            for (var i = 0; i < 3; i++) {
                selectInput[i].setAttribute("disabled", "true");
            }
            selectTextarea[0].setAttribute("disabled", "true");
            selectSelect[0].setAttribute("disabled", "true");
            $(".custom button").attr("disabled", true);
        }
    }
}
//26.2添加下一个
function addNext(data,treeType){
    ...
    if (arr.length != 0 || selectTextarea[0].value === "" || selectSelect[0].value === "0") {//如果没填全，弹出提示框
        tipWarning(tipC, tip1, tipP, "请先全部填写要处理的数据信息！");
    } else {
        $.each(data, function (i, n) {//判断是否已经存在重复内容
            (n.name === selectInput[0].value) && (information = true);
        });
        if (information) {//存在重复内容，弹出提示框
            tipWarning(tipC, tip1, tipP, "已存在重复数据信息！");
        } else {
            if (typeArr.indexOf(selectInput[1].value) == -1) {//判断是否存在重复的类型名
                data.push({
                    "id": id,
                    "pId": 0,
                    "name": selectInput[1].value,//类型名称
                    "icon": "imgs/icon04.png",
                    "isParent": true
                });
                typeArr[id] = selectInput[1].value;
                id++;
            }
            while (idArr.indexOf(cId) != -1) { cId = random(10000, 99999); }//生成一个不重复的id号
            data.push({
                "id": cId,
                "pId": typeArr.indexOf(selectInput[1].value),//类型id
                "name": selectInput[0].value,//特征名称
                "icon": "imgs/icon02.png",
                "operation": selectInput[2].value,//检测值
                "l": selectSelect[0].value,//危害级别
                "ty": selectInput[1].value,//攻击类型
                "detail": selectTextarea[0].value//详细描述
            });
            $.fn.zTree.init(treeType, setting, data);//初始化自定义特征库树
            //清空输入框
            for (i = 0; i < 3; i++) {
                selectInput[i].value = "";
            }
            selectTextarea[0].value = "";
            selectSelect[0].value = 0;
        }
    }
}
//26.3操作列表-添加按钮
function customAdd(){
    ...
    selectSelect[0].value = 0;
    selectSelect[0].removeAttribute("disabled");
    $(".custom button").attr("disabled", false);
}

//27.添加弹出框
//27.1select的禁用
function popSelect(me,showData){
    var text=me[0].labels[0].innerText,
        select=$(".pop-select"),
    ...
    if(text.indexOf("复")==-1){
        ...
        $.each(showData.data,function(i,n){
            str+='<option value="'
            +(i+1)+'">'+ n.name+'</option>';
        });
        select.html(str);
...
function popAdd(){
    ...
    tipAll.hide();
    var contentHeight=$(".page-content>.tip-content").height(),
        panHeight=$(this).parents(".page-content").height();
    (contentHeight>panHeight)? tipC.height(contentHeight):tipC.height(panHeight);
    pop.show();
}