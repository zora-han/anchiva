...
function hInformation(){
    ...
    var subtraction=arguments[1]-arguments[0];
    switch (arguments.length){
        ...
        case 1:
            ...
            break;
        case 2:
            interval=subtraction<3600*1000?30000//时间差小于1个小时 30秒一个数据
                :subtraction<24*3600*1000?60000//时间差小于1天 1分钟一个数据
                :subtraction<30*24*3600*1000?3600000//时间差小于1月 1小时一个数据
                :(24*3600*1000);//时间差大于1月 1天一个数据
            count=(subtraction)/interval;
    ...
    geng();
    HInformationChart.timeTicket = setInterval(geng, 600);
    if(arguments.length>0){
        clearInterval(HInformationChart.timeTicket);
    }
...
function httpConcurrent(){
    ...
    gengHC();
    HConcurrentChart.timeTicket = setInterval(gengHC, 2000);
    if(arguments.length>0){
        clearInterval(HConcurrentChart.timeTicket);
    }
...
function httpCreate(){
    ...
    gengHC();
    HCreateChart.timeTicket = setInterval(gengHC, 2000);
    if(arguments.length>0){
        clearInterval(HCreateChart.timeTicket);
    }
    window.addEventListener("resize",HCreateChart.resize);
}
//4.6攻击地图
function attackMap(){
    var attackMapChart = echarts.init(document.getElementById('attack-map'));
    var geoCoordMap = {
        '上海': [121.4648,31.2891],
        '东莞': [113.8953,22.901],
        '东营': [118.7073,37.5513],
...
function attackMap1(){
    var attackMapChart = echarts.init(document.getElementById('attack-map1'));
    var geoCoordMap = {
        '上海': [121.4648,31.2891],
        '东莞': [113.8953,22.901],
        '东营': [118.7073,37.5513],
...
//4.7威胁安全趋势
function securityTrend1(){
    var securityTrendChart = echarts.init(document.getElementById('security-trend1'));
    var interval,count,now;
    var subtraction=arguments[1]-arguments[0];
    switch (arguments.length){
        case 0:
    ...
    gengS();
    securityTrendChart.timeTicket = setInterval(gengS, 2000);
    if(arguments.length>0){
        clearInterval(securityTrendChart.timeTicket);
    }
    window.addEventListener("resize",securityTrendChart.resize);
}
function securityTrend(){
    var securityTrendChart = echarts.init(document.getElementById('security-trend'));
    var interval,count,now;
    var subtraction=arguments[1]-arguments[0];
    switch (arguments.length){
        case 0:
...
//4.8攻击源排名
function attackSourceRanking1(){
    var attackSourceChart = echarts.init(document.getElementById('attack-source-ranking1'));
...
function attackSourceRanking(){
    var attackSourceChart = echarts.init(document.getElementById('attack-source-ranking'));
    var attackSource=tab("attackSource");
    var ip=[],data=[];
    $.each(attackSource.data,function(i,n){
        ip.push(n.ip);
...
//4.9攻击目的排名
function attackPurposesRanking1(){
    var attackPurposesChart = echarts.init(document.getElementById('attack-purposes-ranking1'));
...
function attackPurposesRanking(){
    var attackPurposesChart = echarts.init(document.getElementById('attack-purposes-ranking'));
    var attackSource=tab("attackPurposes");
    var ip=[],data=[];
    $.each(attackSource.data,function(i,n){
        ip.push(n.ip);
...
function dataPicker(){
    $(".form_datetime").datetimepicker({
        ...
        autoclose: true,
        todayBtn: true,
        startDate:new Date(new Date()-90*24*3600*1000).toLocaleString().replace(/\//g,'-').replace(/[上下午]{2}/,""),
        endDate: new Date().toLocaleString().replace(/\//g,'-').replace(/[上下午]{2}/,""),
        minuteStep: 10
    });
}
function timePicker(href){
    $("#"+href+" "+"[data-submit='true']").on("click",function(){
        var div=$(this).parent().prev().children();
        var time=[];
        $.each(div,function(i,n){
            time.push(n.children[0].value.replace(/-/g,"/"));
        });
        var start=new Date(time[0]);
        var end=new Date(time[1]);
        if(time[0]!==""&&time[1]!==""){
            if(href==="monitoring-defense"){
                securityTrend(start,end);
            }else if(href==="WAF"){
                securityTrend1(start,end);
                console.log(45);
            }else if(href==="system-state"){
                hInformation(start,end);
            }else if(href==="session-information"){
                httpConcurrent(start,end);
                httpCreate(start,end);
            }
            $.each(div,function(i,n){
                n.children[0].value="";
            });
...
function timePicker1(href){
    $("#"+href+" "+"[data-submit='true1']").on("click",function(){
        var div=$(this).parent().prev().children();
        var time=[];
        $.each(div,function(i,n){
            time.push(n.children[0].value.replace(/-/g,"/"));
