...
function hInformation(){
    ...
    switch (arguments.length){
        ...
        case 2:
            ...
            now=new Date(new Date()-arguments[1]);
    }
    var date = [];
    var data = [];
    var data1= [];
    function addData(shift) {
        date.push(now.toLocaleString().replace(/[上下午]{2}/,''));
...
function httpConcurrent(){
    ...
    function addData(shift) {
        if(interval<=3600000){
            ...
            date.push(now.toLocaleDateString().replace(/^201[0-9]\//,''));
        }
...
function securityTrend1(){
    ...
    var date = [];
    var data = [];
    var now=new Date(new Date()-3600000);
    function addData(shift) {
        date.push(now.toLocaleTimeString().replace(/[上下午]{2}/,""));
        data.push(random(0,999999));
        if (shift) {
        ...
        now = new Date(+new Date(now) + 60000);
    }
    for (var i = 1; i < 60; i++) {addData();}
    optionSecurityTrend = {
        tooltip: {
            trigger: 'axis'
        },
    ...
    securityTrendChart.timeTicket = setInterval(function () {
        addData(true);
